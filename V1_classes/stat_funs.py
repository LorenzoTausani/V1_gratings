...
#stats functions
def get_mean_sem(phys_rec: NDArray, s_obj, id : str):
    """
    ...
    Mean_SEM_dict = {}
    for key in s_obj.data[id]['logical_dict'].keys():
        stim_rec = s_obj.get_recording(stim_name = key,ld_id = id, phys_rec = phys_rec)
    ...
    s_obj.data[id]['mean_sem'] = Mean_SEM_dict
    
    
def trace_goodness(phys_rec: NDArray, s_obj = None, id : str|None = None) -> NDArray:
    ...
    if s_obj:
        s_obj.data[id]['t_goodness'] = goodness
    return goodness
    
    
def get_OSI(phys_rec: NDArray, s_obj, id : str):
    """
    ...
    l_dict = s_obj.data[id]['logical_dict']; s_time = s_obj.analysis_settings['stim_duration']
    ...
    s_obj.data[id]['OSI_delta_st_pre'] = {}; s_obj.data[id]['OSI_tuningC_avg'] = {}
    s_obj.data[id]['OSI_tuningC_sem'] = {}
    for _, k in enumerate(k_ori): #per ogni orientamento...
        ori_rec = s_obj.get_recording(stim_name = k, phys_rec = phys_rec, ld_id = id,
                      stim_time = s_time, get_pre_stim = False, latency=l)
        prestim_rec = s_obj.get_recording(stim_name = k, phys_rec = phys_rec, ld_id = id,
                      stim_time = s_time, get_pre_stim = True, latency=l)
        
        avg_st = np.mean(ori_rec, axis = 2); avg_pre = np.mean(prestim_rec, axis = 2)
        
        s_obj.data[id]['OSI_delta_st_pre'][k] = (avg_st-avg_pre)/avg_pre
        s_obj.data[id]['OSI_tuningC_avg'][k] = np.nanmean(s_obj.data[id]['OSI_delta_st_pre'][k],axis=0); 
        s_obj.data[id]['OSI_tuningC_sem'][k] = SEMf(s_obj.data[id]['OSI_delta_st_pre'][k])
        
    s_obj.data[id]['delta_gray_avg'] = np.mean([np.mean(v, axis = 0)*100 for _,v 
                              in s_obj.data[id]['OSI_delta_st_pre'].items()], axis=0)
    s_obj.data[id]['OSI_tuningC_df'] = compute_OSI(s_obj.data[id]['OSI_tuningC_avg'])
